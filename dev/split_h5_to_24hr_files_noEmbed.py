...
def split_h5_to_24hr_files(input_filename, out_dir, time_ds_path='/data/time'):
    ...
    with h5py.File(input_filename, 'r') as h5in:
        ...
        for segment_start, segment_end in intervals:
            #First, check to see if a file for this segment already exists, and skip if so (to save time when re-running)
            outfn = os.path.join(
                out_dir, 
            ...
            #If the file does not yet exist, slice the data and save the file:
